...
<body>
 ...
 %> <%=boardBoList.get(i).getD_author()%> :
					...
					class="smalltext"
					onclick="location.href='datDelete.bo?b_division=<%=b_division%>&d_code=<%=boardBoList.get(i).getD_code()%>&b_code=<%=boardBoList.get(i).getD_boardcode()%>'">삭제</a>&nbsp;
					<%
						}
					%>
					<form method="post"
	...
	<section id="commandArea">
		...
		%>
		<a
			href="boardRemakeForm.bo?b_code=<%=board.getB_code()%>&b_content=<%=board.getB_content()%>&b_name=<%=board.getB_name()%>&b_mcode=<%=board.getB_mcode()%>&b_division=<%=board.getB_division()%>"
			id="contentsujung" class="smalltext">수정</a>&nbsp; <a
			id="contentdelete" class="smalltext"
			onclick="boardDelete(<%=board.getB_code()%>,<%=b_division%>)">삭제</a>&nbsp;
		<%
			}
		%>
		<%
